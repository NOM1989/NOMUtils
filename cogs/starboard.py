        ...
        if payload.guild_id and payload.guild_id == self.starboard_guild_id and payload.emoji.name == '⭐' and payload.channel_id != options['starboard']['channel']: #ignore the starboard channel
            ...
            if index not in starboard_data:
                if star_count >= self.low_stars:
                    ...
                    sent_message_1 = await self.starboard_webhook.send(content=((message.content[:1972] + '..') if len(message.content) > 1974 else message.content), files=files, embeds=message.embeds, username=message.author.display_name, avatar_url=message.author.display_avatar.url, allowed_mentions=discord.AllowedMentions.none(), wait=True)
