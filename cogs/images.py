...
class Images(commands.Cog):
    ...
    @commands.command()
    async def wanted(
        self, ctx: Context, who: Union[discord.Member, discord.User] = None, *text: str
    ):
        AVATAR_SIZE = 396
        AVATAR_BORDER = 4
        ...
        async with ctx.typing():
            ...
            with BytesIO() as buffer:
                ...
                avatar.alpha_composite(
                    Image.new("RGBA", avatar.size, (238, 214, 132, 123))
                )  # Add transparent colour overaly

                # Resize avatar image
                avatar = avatar.resize(
                    (AVATAR_SIZE, AVATAR_SIZE), resample=Image.Resampling.BICUBIC
                )
                # Adjust transparency
                avatar.putalpha(170)
                ...
                draw.text(
                    ((wanted.width - w) / 2, 616), text, font=font, fill=(5, 1, 4, 240)
                )
                ...
                draw.rounded_rectangle(
                    (
                        (x - AVATAR_BORDER, y - AVATAR_BORDER),
                        (
                            x + AVATAR_SIZE + AVATAR_BORDER - 1,
                            y + AVATAR_SIZE + AVATAR_BORDER - 1,
                        ),
                    ),
                    outline=(40, 20, 0, 120),
                    width=AVATAR_BORDER,
                    radius=AVATAR_BORDER,
                )
