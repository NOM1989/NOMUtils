...
class Public(commands.Cog):
    ...
    async def avatar_message(self, ctx, who, avatar):
        ...
        async with ctx.typing():
            with BytesIO() as buffer:
                ...
                avatar_file = discord.File(buffer, filename=filename)
                embed = discord.Embed(
                    description=f"{who.mention}'s Avatar", colour=0x2F3136
                )
                embed.set_image(url=f"attachment://{filename}")
                try:
    ...
    async def avatar(self, ctx, *, who: Union[discord.Member, discord.User] = None):
        ...
        if who.avatar is not None and who.display_avatar != who.avatar:
            try:
    ...
    async def it(self, ctx, *args):
        ...
        day_specials = {
            "friday": (
                "yes",
                "among us fri-yay?! :partying_face:",
                "yes... <:sus:822637858900148256>?",
            ),
        ...
        if len(args) > 0:
            ...
            if possible_match:
                ...
                if day_today == possible_match[0]:
                    await ctx.send(
                        choice(day_specials[day_today])
                        if day_today in day_specials
                        else "yes"
                    )
                else:
                    await ctx.send("no")
    ...
    async def search(self, ctx: Context, *, name_or_tag: str):
        ...
        possible_channel = get_close_matches(
            name_or_tag,
            [getattr(channel, "name") for channel in ctx.guild.text_channels],
            n=1,
        )
        if possible_channel:
            channel = discord.utils.get(
                ctx.guild.text_channels, name=possible_channel[0]
            )
            perms: discord.Permissions = channel.permissions_for(ctx.message.author)
            if perms.read_messages:
                to_delete.append(
                    await ctx.reply(
                        channel.mention, allowed_mentions=discord.AllowedMentions.none()
                    )
                )
        if len(to_delete) == 1:
