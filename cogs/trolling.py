...
class Trolling(commands.Cog):
    ...
    async def on_guild_channel_create(self, channel: discord.TextChannel):
        ...
        if channel.guild.id not in self.exempt_guilds and isinstance(
    ...
    async def on_voice_state_update(
        ...
        if (
            member.guild.id not in self.exempt_guilds
            and member.id not in self.exempt_members
        ):
            ...
            if before.channel != after.channel:
                ...
                if (
                    ...
                    and before.channel.members[0].voice.self_stream
                ):
                    await self._set_nickname(
                        before.channel.members[0], overwrite=True
                    )  # no views
            ...
            # Account for start stream
                ...
                and member.id not in self.tmp_name_store
            ):
